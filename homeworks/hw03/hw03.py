...
def balanced(m):
    ...
    if is_planet(m):
        ...
        return left(m)*total_weight(left(m)) == right(m)*total_weight(right(m)) and balanced(left(m)) and balanced(right(m))
...
def totals_tree(m):
    ...
    if is_mobile(m):
        t = [total_weight(m)]
        t.append(totals_tree(left(m)))
        t.append(totals_tree(right(m)))
        return t
    if is_arm(m):
