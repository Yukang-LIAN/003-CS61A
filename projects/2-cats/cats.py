...
def accuracy(typed, reference):
    ...
    for x, y in zip(typed_words, reference_words):
        if x == y:
            same += 1
    return 100*same/len(typed_words)
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    min_word = ""
    for x in valid_words:
        num = diff_function(user_word, x, limit)
...
def pawssible_patches(start, goal, limit):
    ...
    def helper(start, goal, limit, num):
        ...
        if start[0] != goal[0]:
            num += 1
            return min(helper(start[1:], goal[1:], limit, num), helper(start, goal[1:], limit, num), helper(start[1:], goal, limit, num))
    return helper(start, goal, limit, 0)
